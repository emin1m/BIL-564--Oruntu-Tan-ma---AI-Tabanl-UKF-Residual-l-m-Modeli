...
UKF Tracking with a GRU-Corrected Measurement Model
...
shared GRU that processes the sensor feature sequence and predicts one
residual value per sensor.
"""
...
class ResidualPerSensorGRU(nn.Module):
    """
    Input per sensor: [a_i, d2_i, x, y, vx, vy, xi, yi] -> residual measurement
    The sensor dimension is treated as a sequence and the GRU emits one residual
    per sensor step.
    """

    def __init__(self, in_dim: int = 8, hidden: int = 128, num_layers: int = 1, dropout: float = 0.0):
        super().__init__()
        self.gru = nn.GRU(
            input_size=in_dim,
            hidden_size=hidden,
...
def residual_features_torch(x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Build residual features for each sensor:
    ...
    sc = residual_mod.sensor_coords_t(x.device).unsqueeze(0).expand(x.shape[0], -1, -1)
    x_rep = x.unsqueeze(1).expand(-1, residual_mod.num_sensors, -1)
    feats = torch.cat([a.unsqueeze(-1), d2.unsqueeze(-1), x_rep, sc], dim=-1)
    return a, feats


@torch.no_grad()
def hx_meas_hybrid_gru(
    x_np: np.ndarray,
    net: nn.Module,
    device: str = "cpu",
) -> np.ndarray:
...
def train_residual_gru_net(
    ...
    def make_predictions(x_batch: torch.Tensor) -> torch.Tensor:
        a, feats = residual_features_torch(x_batch)
        residual = net(feats)
        return a + residual
...
def main() -> None:
    ...
    print("Training residual measurement GRU...")
    net = train_residual_gru_net(
        X_train,
        Z_train,
        X_val,
        Z_val,
    ...
    def hx_hybrid_gru(x_np: np.ndarray) -> np.ndarray:
        return hx_meas_hybrid_gru(x_np, net=net, device=device)
    ...
    print("Running UKF (hybrid physics + GRU residual)...")
    ...
    plt.plot(xs_est_hyb[:, 0], xs_est_hyb[:, 1], color="tab:orange", linewidth=2.0, label="UKF (hybrid physics+GRU)")
    plt.scatter(
    ...
    plt.title("Continuous Sensor Tracking: UKF with Physics vs Hybrid Physics+GRU")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.grid(True)
    plt.axis("equal")
    ...
    plt.plot(err_hyb, color="tab:orange", label="pos error (hybrid GRU)")
