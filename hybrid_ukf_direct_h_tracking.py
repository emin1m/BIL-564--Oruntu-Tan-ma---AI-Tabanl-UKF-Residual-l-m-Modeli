"""
Directly Learned Measurement Model h(x) for Continuous Sensors
UKF Tracking with a Neural Measurement Function (No Particle Filter)

What this script does (end-to-end):
1) Keeps your physics simulation setup for generating synthetic measurements
2) Uses a mismatched continuous sensor response to generate train/test data
3) Trains a neural net h_theta(x, sensor_i) to predict each sensor measurement directly
4) Runs UKF where hx(x) is either:
   - physics-only amplitude model
   - directly learned neural measurement model h_theta
5) Compares tracking performance of both UKF variants
"""
...
# -----------------------------
# Optional: PyTorch (required for learned h)
# -----------------------------
try:
    import torch
...
# ============================================================
# 4) Direct neural measurement model h_theta(phi)
#    Shared per-sensor MLP: phi_i(x) -> predicted_measurement_i
# ============================================================
class DirectHPerSensorMLP(nn.Module):
    """
    Input per sensor: [d2_i, x, y, vx, vy, xi, yi] -> predicted measurement
    Shared weights across sensors, outputs 25 measurements for a batch.
    """
    ...
    def forward(self, feats: torch.Tensor) -> torch.Tensor:
        # feats: (B, N, in_dim) -> predictions: (B, N)
        B, N, D = feats.shape
        out = self.net(feats.view(B * N, D))
        return out.view(B, N)


def direct_features_torch(x: torch.Tensor) -> torch.Tensor:
    """
    Build direct features for each sensor:
      [d2_i, x, y, vx, vy, xi, yi]
    x: (B,4)
    returns feats: (B,25,7)
    """
    B = x.shape[0]
    sc = sensor_coords_t(x.device).unsqueeze(0).expand(B, -1, -1)  # (B,25,2)
    x_rep = x.unsqueeze(1).expand(-1, num_sensors, -1)  # (B,25,4)
    diff = sc - x[:, 0:2].unsqueeze(1)  # (B,25,2)
    d2 = (diff ** 2).sum(dim=-1, keepdim=True)  # (B,25,1)
    feats = torch.cat([d2, x_rep, sc], dim=-1)  # (B,25,7)
    return feats
...
def hx_meas_direct(
    ...
    """
    Direct neural measurement model:
      z_hat(x) = h_theta(x)
    returns (25,) numpy
    """
    x = torch.tensor(np.asarray(x_np, dtype=np.float32).reshape(1, 4), device=device)
    feats = direct_features_torch(x)  # (1,25,7)
    z_pred = net(feats).squeeze(0)  # (25,)
...
# ============================================================
# 6) Train direct h net (MSELoss)
# ============================================================
def train_direct_h_net(
    X_train: np.ndarray,
    Z_train: np.ndarray,
    X_val: np.ndarray,
    Z_val: np.ndarray,
    ...
    net = DirectHPerSensorMLP(in_dim=7, hidden=128).to(device)
    ...
    def make_predictions(x_batch: torch.Tensor) -> torch.Tensor:
        feats = direct_features_torch(x_batch)  # (B,25,7)
        return net(feats)  # (B,25)
...
# ============================================================
# 7) Tracking experiment: compare physics-only vs direct-h (AI) UKF
# ============================================================
...
if __name__ == "__main__":
    ...
    print("Training direct h(x) measurement net...")
    net = train_direct_h_net(
    ...
    x0_est = default_filter_initial_state_for_trajectory(
        trajectory_kind=trajectory_kind,
        cv_x0_est=np.array([-7.0, 9.0, 1.0, -0.7], dtype=np.float64),
    )
    ...
    # hx for direct-h UKF (pure neural measurement model)
    def hx_direct(x_np: np.ndarray) -> np.ndarray:
        return hx_meas_direct(x_np, net=net, device=device)

    print("Running UKF (physics-only)...")
    xs_est_phys = run_ukf_tracking(xs_true, zs_meas, dt, Q, x0_est, P_init, hx_phys, meas_std=meas_std)

    print("Running UKF (direct learned h)...")
    xs_est_direct = run_ukf_tracking(xs_true, zs_meas, dt, Q, x0_est, P_init, hx_direct, meas_std=meas_std)

    rmse_phys = rmse_pos(xs_est_phys, xs_true)
    rmse_direct = rmse_pos(xs_est_direct, xs_true)
    print(f"Position RMSE | physics-only UKF: {rmse_phys:.4f}")
    print(f"Position RMSE | direct-h UKF    : {rmse_direct:.4f}")
    ...
    plt.plot(xs_est_direct[:, 0], xs_est_direct[:, 1], "g-", linewidth=2.0, label="UKF (direct learned h)")

    plt.scatter(sensor_coords[:, 0], sensor_coords[:, 1], s=80, marker="^", alpha=0.7, label="Sensors")

    plt.title("Continuous Sensor Tracking: UKF with Physics vs Direct Learned h")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.grid(True)
    plt.axis("equal")
    ...
    err_direct = np.linalg.norm(xs_est_direct[:, :2] - xs_true[:, :2], axis=1)

    plt.figure(figsize=(10, 4))
    plt.plot(err_phys, "k--", label="pos error (physics-only)")
    plt.plot(err_direct, "g-", label="pos error (direct-h)")
